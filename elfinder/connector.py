...
class ElFinderConnector():
    ...
    def get_commands(self):
        ...
        return {'open': ('__open', {'target': True}),
                ...
                'download': ('__download', {'targets[]': True}),
    ...
    def __download(self):
        ...
        Writes the requested files into the response.
        TODO: Set encoding correctly
        """
        for target in self.data['targets[]']:
            target = self.data['target']
            volume = self.get_volume(target)
            faaa = volume.get_object(target)
            self.httpHeader['Content-type']='application/octet-stream'
            self.httpHeader['Content-Disposition']='attachment; filename='+str(faaa.name)
            self.httpHeader['Content-Location']='trt'
            self.httpHeader['Content-Transfer-Encoding']='binary'
            self.response=faaa.content.read()
            self.httpHeader['Content-Length']=faaa.content.size
