...
class Directory(MPTTModel, FileCollectionChildMixin):
    ...
    def delete(self, *args, **kwargs):
        ...
        for item in self.files.all():
            item.delete()
        super(Directory, self).delete(*args, **kwargs) # Call the "real" save() method.
...
def content_file_name(instance, filename):
    ancestors=filename.split("$$$$")[::-1]
    current=instance
    while current.parent:
        ancestors.append(current.parent.name)
        current=current.parent
    ancestors.pop()
    ancestors.append("materials")
    result='/'.join(ancestors[::-1])
...
class File(models.Model, FileCollectionChildMixin):
    ...
    def delete(self, *args, **kwargs):
        self.content.delete()
        super(File, self).delete(*args, **kwargs) # Call the "real" delete() method.
    class Meta:
        unique_together = ('name', 'parent')
